...
def index(request):
    ...
    cntxt = {'cartitems':cartitems, 'total_price': total_price, 'userprofile': userprofile}
    return render(request, 'store/checkout.html', cntxt)

@login_required(login_url='loginpage')
def placeorder(request):
    if request.method == "POST":

        curruser = User.objects.filter(id=request.user.id).first()
        ...
        trackno = 'azybcx'+ str(random.randint(1111111,9999999))
        while Order.objects.filter(tracking_no=trackno) is None:
            trackno = 'azybcx'+ str(random.randint(1111111,9999999))
        neworder.tracking_no = trackno
        neworder.save()

        newordeitems = Cart.objects.filter(user=request.user)
        for item in newordeitems:
            ...
            # decrease the quantity of prdct from stock
